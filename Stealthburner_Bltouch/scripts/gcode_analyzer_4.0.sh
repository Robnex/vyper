...
show_coords=true # true = enabled / false = diabled
...
# if needed do "sudo chown gcode_analyzer_xxx.sh" on this file

#############################
# Dont touch the area below #

limitx=165    #set the limit of X axis
limity=210    #set the limit of y axis
...
# show conflicts on y
grep "Y[2][1][0-9]" $filepath > $y_conflict
...
# show conflicts on x
echo -e "--------------------------------"
grep -e "X[1][6-9]" $y_conflict > $x_conflict
